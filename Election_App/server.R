...
states_gen_ratio_data <- do.call(rbind, states_gen_ratio_list)
  ...
  output$plot <- renderPlot({
    states_gen_ratio_data %>% subset(st_name == input$state) %>%
      ggplot(aes(x = year, y = gen_ratio, group = st_name, col = st_name)) + 
      coord_cartesian(ylim = c(0, input$y_axis), xlim = c(1977, input$year)) + geom_point(aes(size = gen_ratio,)) + 
      geom_smooth(se = FALSE) + labs( x = "Year", y = "Gender Ratio (Female Candidates per 100  Male Candidates)") +
